...
export function WxIcon({ kind = 'clear-day', size = 24, color, stroke: sw = 1.6 }: Props) {
  ...
  switch (kind) {
    case 'clear-day': return (
      <svg viewBox="0 0 24 24" style={s} fill="none">
        ...
        {[0, 45, 90, 135, 180, 225, 270, 315].map(a => {
          const x1 = 12 + Math.cos(a * Math.PI / 180) * 6.5;
          const y1 = 12 + Math.sin(a * Math.PI / 180) * 6.5;
          const x2 = 12 + Math.cos(a * Math.PI / 180) * 8.5;
          const y2 = 12 + Math.sin(a * Math.PI / 180) * 8.5;
          return <line key={a} x1={x1} y1={y1} x2={x2} y2={y2} stroke={c} strokeWidth={sw} strokeLinecap="round" />;
        })}
      </svg>
    );
    case 'clear-night': return (
      <svg viewBox="0 0 24 24" style={s} fill="none">{P('M18 14.5A7 7 0 0 1 9.5 6a7 7 0 1 0 8.5 8.5z')}</svg>
    );
    case 'partly-cloudy-day': return (
      <svg viewBox="0 0 24 24" style={s} fill="none">
        <circle cx="8" cy="9" r="3" stroke={c} strokeWidth={sw} fill="none" />
        {[-45, 0, 45, -90].map(a => {
          const x1 = 8 + Math.cos(a * Math.PI / 180) * 4.5;
          const y1 = 9 + Math.sin(a * Math.PI / 180) * 4.5;
          const x2 = 8 + Math.cos(a * Math.PI / 180) * 6;
          const y2 = 9 + Math.sin(a * Math.PI / 180) * 6;
          return <line key={a} x1={x1} y1={y1} x2={x2} y2={y2} stroke={c} strokeWidth={sw} strokeLinecap="round" />;
        })}
    ...
    case 'cloudy': return (
      <svg viewBox="0 0 24 24" style={s} fill="none">{P('M6 17a3.5 3.5 0 0 1 0-7 5 5 0 0 1 9.5 0.5A3.5 3.5 0 0 1 17 17H6z')}</svg>
    );
    case 'rain': return (
      <svg viewBox="0 0 24 24" style={s} fill="none">
    ...
    case 'fog': return (
      <svg viewBox="0 0 24 24" style={s} fill="none">{P('M3 9h18M5 13h14M3 17h18')}</svg>
    );
    case 'wind': return (
      <svg viewBox="0 0 24 24" style={s} fill="none">{P('M3 8h13a2.5 2.5 0 1 0-2.5-2.5M3 12h17a3 3 0 1 1-3 3M3 16h9')}</svg>
    );
...
export function forecastToCondition(text: string): WxCondition {
  ...
  if (t.includes('snow') || t.includes('blizzard') || t.includes('flurr')) return 'snow';
  ...
  if (t.includes('cloudy') && t.includes('partly')) return 'partly-cloudy-day';
  if (t.includes('cloudy') || t.includes('overcast') || t.includes('mostly cloudy')) return 'cloudy';
